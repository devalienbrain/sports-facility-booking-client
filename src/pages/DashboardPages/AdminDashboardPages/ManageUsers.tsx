...
const ManageUsers: React.FC = () => {
  ...
  const handleAdminToggle = (id: string, isAdmin: boolean) => {
    if (
      ...
      // Logic to toggle admin status can go here
      console.log(isAdmin ? "Removing admin rights" : "Making admin", id);
      alert(`Admin status ${isAdmin ? "removed" : "granted"} successfully`);
      refetch(); // Refetch users after the change
    }
  };
...
// const ManageUsers: React.FC = () => {
//   // Use the return type correctly to avoid type errors
//   const { data: users, error, isLoading, refetch } = useGetAllUsersQuery();
//   const [deleteUser] = useDeleteUserMutation();
