...
const ManageBookings: React.FC = () => {
  ...
  const allBookings: TBooking[] = bookings?.data || [];
  const handleDelete = async (id: string) => {
    if (window.confirm("Are you sure you want to cancel this booking?")) {
      try {
        await deleteBooking(id).unwrap();
        alert("Booking canceled successfully");
        refetch(); // Refetch bookings after deletion
      } catch (err) {
          ...
          {allBookings.map((booking: TBooking, index) => (
            <tr key={booking._id} className="hover:bg-gray-100 transition-all">
              ...
              <td className="py-3 px-6 border-b flex items-center space-x-4">
                <Link to={`/bookings/edit/${booking._id}`} className="text-blue-600 hover:text-blue-800">
                  <FaEdit />
                </Link>
                <button
